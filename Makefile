...
URL_PATH_NETDATA_REGISTRY  := "http://influxdb-master-01.$(DNSMASQ_DOMAIN):19999"
URL_PATH_NETDATA_NODE      := "http://influxdb-worker-01.$(DNSMASQ_DOMAIN):19999"
...
run-ansible-influxdb:
	@ansible-playbook -i inventory.ini influxdb_playbook.yml -v
...
ssh-bridge-master:
	ssh -vvvv -F ./ssh_config influxdb-master-01.scarlettlab.home

ssh-bridge-worker:
	ssh -vvvv -F ./ssh_config influxdb-worker-01.scarlettlab.home
